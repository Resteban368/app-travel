...
class PoliticaReservaListScreen extends StatefulWidget {
  ...
  @override
  State<PoliticaReservaListScreen> createState() => _PoliticaReservaListScreenState();
}
...
class _PoliticaReservaListScreenState extends State<PoliticaReservaListScreen>
          ...
          children: [
            ...
            BlocBuilder<PoliticaReservaBloc, PoliticaReservaState>(
              builder: (context, state) {
                ...
                final canWrite = authState is AuthAuthenticated && authState.user.canWrite('politicasReserva');
                ...
                return CustomScrollView(
                  ...
                  slivers: [
                    ...
                    SliverFadeTransition(
                      ...
                      sliver: _buildContent(context, state, politicas, canWrite),
                    ),
  ...
  Widget _buildContent(BuildContext context, PoliticaReservaState state, List<PoliticaReserva>? politicas, bool isAdmin) {
    ...
    final filtered = politicas.where((p) => p.titulo.toLowerCase().contains(_searchQuery.toLowerCase()) || p.descripcion.toLowerCase().contains(_searchQuery.toLowerCase())).toList();
    ...
    return SliverPadding(
      ...
      sliver: SliverList(
        delegate: SliverChildBuilderDelegate((context, index) {
          ...
          return _PoliticaCard(politica: politica, isAdmin: isAdmin, state: state);
...
class _PoliticaCard extends StatefulWidget {
  ...
  const _PoliticaCard({required this.politica, required this.isAdmin, required this.state});
  ...
  void _confirmDelete() {
    showDialog(
      ...
      builder: (ctx) => _PremiumConfirmDialog(
        ...
        content: 'Esta acción borrará "${widget.politica.titulo}" permanentemente.',
        onConfirm: () {
          context.read<PoliticaReservaBloc>().add(DeletePolitica(widget.politica.id));
          Navigator.pop(ctx);
        },
      ),
    );
  ...
  Widget build(BuildContext context) {
    ...
    return MouseRegion(
      ...
      child: AnimatedContainer(
        ...
        child: InkWell(
          onTap: () => Navigator.pushNamed(context, AppRouter.politicaReservaEdit, arguments: p),
          borderRadius: BorderRadius.circular(22),
          child: Padding(
            ...
            child: Row(
              ...
              children: [
                Container(
                  ...
                  child: const Icon(Icons.policy_rounded, color: D.royalBlue, size: 24),
                ),
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      Row(
                        children: [
                          Expanded(child: Text(p.titulo, style: const TextStyle(color: Colors.white, fontSize: 16, fontWeight: FontWeight.bold))),
                          if (!p.activo) _Tag(label: 'INACTIVO', color: D.slate600),
                        ],
                      ),
                      const SizedBox(height: 8),
                      Text(p.descripcion, maxLines: 2, overflow: TextOverflow.ellipsis, style: TextStyle(color: D.slate400, fontSize: 13)),
                      const SizedBox(height: 12),
                      Row(
                        children: [
    ...
    return Row(
      ...
      children: [
        Transform.scale(
          scale: 0.8,
          child: Switch(
            value: widget.politica.activo,
            activeThumbColor: D.royalBlue,
            onChanged: isSaving ? null : (v) => context.read<PoliticaReservaBloc>().add(TogglePoliticaActive(widget.politica.id)),
          ),
        ),
        IconButton(icon: const Icon(Icons.delete_outline_rounded, color: D.rose, size: 20), onPressed: isSaving ? null : _confirmDelete),
      ],
    );
  }
}
...
class _Tag extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) => Container(padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4), decoration: BoxDecoration(color: color, borderRadius: BorderRadius.circular(6)), child: Text(label, style: const TextStyle(color: Colors.white, fontSize: 10, fontWeight: FontWeight.bold)));
}
...
class _TypeBadge extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) => Container(padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 4), decoration: BoxDecoration(color: D.bg.withOpacity(0.5), borderRadius: BorderRadius.circular(8), border: Border.all(color: D.border)), child: Text(type.toUpperCase(), style: TextStyle(color: D.skyBlue, fontSize: 10, fontWeight: FontWeight.w900, letterSpacing: 0.5)));
}
...
class _SkelCard extends StatelessWidget {
  @override
  Widget build(BuildContext context) => Container(height: 120, margin: const EdgeInsets.only(bottom: 16, left: 24, right: 24), decoration: BoxDecoration(color: D.surface.withOpacity(0.5), borderRadius: BorderRadius.circular(22)));
}
...
class _EmptyState extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) => Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [Icon(isSearch ? Icons.search_off_rounded : Icons.policy_outlined, size: 64, color: D.slate800), const SizedBox(height: 16), Text(isSearch ? 'No se encontraron políticas' : 'Sin políticas registradas', style: TextStyle(color: D.slate600, fontSize: 16, fontWeight: FontWeight.bold))]));
}
