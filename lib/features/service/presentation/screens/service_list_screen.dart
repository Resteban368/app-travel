...
class _ServiceListBodyState extends State<_ServiceListBody> with TickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    ...
    final canWrite = authState is AuthAuthenticated && authState.user.canWrite('services');
              ...
              return Stack(
                children: [
                  Positioned(
                    ...
                    child: _Orb(color: D.royalBlue.withOpacity(0.12), size: 350),
                  ),
                  Positioned(
  ...
  Widget _buildSliverContent(ServiceState state, bool canWrite) {
    ...
    if (state is ServicesLoaded) currentServices = state.services;
    else if (state is ServiceSaving && state.services != null) currentServices = state.services;
    else if (state is ServiceSaved && state.services != null) currentServices = state.services;
    ...
    if (currentServices != null) {
      final filtered = currentServices.where((s) => 
    ...
    return AnimatedPadding(
      ...
      child: MouseRegion(
        ...
        child: GestureDetector(
          onTap: () => Navigator.pushNamed(context, AppRouter.serviceEdit, arguments: widget.service),
          child: AnimatedContainer(
            duration: const Duration(milliseconds: 300),
            padding: const EdgeInsets.all(20),
            decoration: BoxDecoration(
              color: widget.service.isActive ? D.surface : D.surface.withOpacity(0.4),
              ...
              boxShadow: _isHovered ? [
                BoxShadow(color: D.royalBlue.withOpacity(0.15), blurRadius: 30, spreadRadius: -5)
              ] : null,
            ),
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
                ...
                Row(
                  ...
                  children: [
                    Container(
                      ...
                      child: Icon(Icons.settings_suggest_rounded, color: D.skyBlue, size: 24),
                    ),
                        ...
                        children: [
                          Text(
                            ...
                            style: TextStyle(
                              ...
                              decoration: widget.service.isActive ? null : TextDecoration.lineThrough,
                            ),
                          ),
                          const SizedBox(height: 4),
                          Text(
                            ...
                            style: TextStyle(color: D.slate400, fontSize: 13, height: 1.4),
                          ),
                        ],
                      ),
                    ),
                ...
                Row(
                  ...
                  children: [
                    Container(
                      ...
                      child: Text(
                        widget.service.cost != null ? currencyFormat.format(widget.service.cost) : 'Gratuito',
                        style: TextStyle(color: D.emerald, fontWeight: FontWeight.w900, fontSize: 15),
                      ),
                    ),
                    if (widget.isAdmin)
                      Row(
                        children: [
                          Transform.scale(
                            ...
                            child: Switch(
                              ...
                              onChanged: (v) => context.read<ServiceBloc>().add(ToggleServiceActive(widget.service.id)),
                            ),
                          ),
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          Text(
            query.isEmpty ? 'Sin servicios adicionales' : 'No hay resultados para "$query"',
            style: const TextStyle(color: Colors.white, fontSize: 18, fontWeight: FontWeight.bold),
          ),
          const SizedBox(height: 8),
          Text(
            query.isEmpty ? 'Los beneficios extras aparecerán aquí' : 'Intenta con otros términos',
